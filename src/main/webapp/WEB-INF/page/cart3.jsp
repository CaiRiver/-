...
<html>
	...
	<body>
		...
		<input id="cardId" type="hidden" name="cardId" value="${bankCard.id }">
	</form>
 	...
 	function settlement() {
 		...
 		$('.th :checkbox').each(function() {
 			if ($(this).is(':checked')) {
 				...
 				buyCommodityArray.push(shoppingTrolleyDTO);
 			}
 		});
 		//var jsonObj = { id: '01', name: 'Tom' };
		//JSON.stringify(jsonObj);
 		var msg = '';
		for (var i=0; i<buyCommodityArray.length; i++) {
			//var jsonObj = { commodityId: '01', purchaseNum: 'Tom' };
			var good = buyCommodityArray[i];
			msg += good.commodityId + '-' + good.purchaseNum + '&';
		}
 	...
 	function listCart() {
		$.ajax({
            ...
            success:function(data,textStatus,jqXHR){
                ...
                cartShow.category = data.data.data;
                ...
                $('#pageInfo2').html('当前页'+data.data.currentPageNum+'/'+data.data.totalPageNum);
